...
class Tetris:
    ...
    def reset(self):
        ...
        # --- 수정된 부분: reset()은 이제 4개 특성 벡터가 아니라 전체 보드를 반환 ---
        # 현재 스폰된 블록도 포함된 전체 보드를 반환해야 함
    ...
    def _get_complete_board(self):
        """
        현재 보드(고정 블록) 위에, 낙하 중인 블록을 piece_id+1 로 오버레이하여
        (20×10) 배열 형태로 반환.
        """
    ...
    def _add_piece_to_board(self, piece, pos):
        """
        고정 블록을 board에 기록하기. piece_id+1 값을 사용.
        """
    ...
    # --- 수정된 부분: get_state_size() → 20×10 = 200 반환 ---
    ...
    def get_next_states(self):
        ...
        그 결과 생기는 상태를 “전체 보드 행렬” (20×10) 형태로 반환하도록 수정.
        """
        states = {}
        ...
        for rot in rotations:
            ...
            for x in range(-min_x, Tetris.BOARD_WIDTH - max_x):
                ...
                if drop_y >= 0:
                    ...
                    # --- 수정: 반환값을 4개 특성 벡터가 아니라 '전체 보드 행렬'로 대체 ---
    ...
    def play(self, x, rotation, render=True, screen=None, scale=25, clock=None):
        ...
        # --- 수정: play()가 더 이상 4개 특성이 아니라 '전체 보드 행렬'을 반환하도록 ---
